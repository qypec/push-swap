...
/*   Updated: 2020/02/07 23:05:44 by yquaro           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void				draw_help_part_1(t_visu *visu, size_t *margin_top)
{
	...
	(*margin_top) += 2;
	mvwprintw(visu->win_help, (*margin_top), MARGIN_LEFT, \
		BUTTON_NAME_NEXT_CYCLE);
	(*margin_top) += 2;
	mvwprintw(visu->win_help, (*margin_top), MARGIN_LEFT, \
		BUTTON_NAME_DEFAULT_SPEED);
	(*margin_top) += 2;
	mvwprintw(visu->win_help, (*margin_top), MARGIN_LEFT, BUTTON_NAME_SPEED_UP);
	(*margin_top) += 2;
	mvwprintw(visu->win_help, (*margin_top), MARGIN_LEFT, \
		BUTTON_NAME_SPEED_DOWN);
}

static void				draw_help_part_2(t_visu *visu, size_t *margin_top)
{
	mvwprintw(visu->win_help, (*margin_top), MARGIN_LEFT + \
		ft_strlen(BUTTON_NAME_SPEED_DOWN), BUTTON_SPEED_DOWN);
	(*margin_top) -= 2;
	mvwprintw(visu->win_help, (*margin_top), MARGIN_LEFT + \
		ft_strlen(BUTTON_NAME_SPEED_UP), BUTTON_SPEED_UP);
	(*margin_top) -= 2;
	mvwprintw(visu->win_help, (*margin_top), MARGIN_LEFT + \
		ft_strlen(BUTTON_NAME_DEFAULT_SPEED), BUTTON_DEFAULT_SPEED);
	(*margin_top) -= 2;
	mvwprintw(visu->win_help, (*margin_top), MARGIN_LEFT + \
		ft_strlen(BUTTON_NAME_NEXT_CYCLE), BUTTON_NEXT_CYCLE);
	(*margin_top) -= 2;
	mvwprintw(visu->win_help, (*margin_top), MARGIN_LEFT + \
		ft_strlen(BUTTON_NAME_PAUSE), BUTTON_PAUSE);
	(*margin_top) -= 2;
	mvwprintw(visu->win_help, (*margin_top), MARGIN_LEFT + \
		ft_strlen(BUTTON_NAME_EXIT), BUTTON_EXIT);
}
...
void					draw_help(t_visu *visu)
{
	...
	draw_help_part_1(visu, &margin_top);
	wcolor_set(visu->win_help, CYAN, NULL);
	draw_help_part_2(visu, &margin_top);
